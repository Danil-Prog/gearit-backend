...
@Repository
public interface AccessPolicyRepository extends JpaRepository<AccessPolicy, Long>, JpaSpecificationExecutor<AccessPolicy> {

    @Query(value = "SELECT accessPolicy FROM AccessPolicy AS accessPolicy WHERE accessPolicy.name = :name")
    AccessPolicy findByName(String name);
}
